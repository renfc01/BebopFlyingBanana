...
import com.parrot.bebopflyingbanana.photo.ARDataTransferMediasDownloaderAvailableMediaCallBack;
import com.parrot.bebopflyingbanana.photo.ARDataTransferMediasDownloaderCompletionCallBack;
import com.parrot.bebopflyingbanana.photo.ARDataTransferMediasDownloaderProgressCallBack;
...
public class DeviceController implements ARCommandCommonCommonStateBatteryStateChangedListener, ARCommandCommonSettingsStateAllSettingsChangedListener, ARCommandCommonCommonStateAllStatesChangedListener, ARCommandARDrone3PilotingStateFlyingStateChangedListener
{
    ...
    private ARDataTransferMediasDownloaderAvailableMediaCallBack availableMediaListener;

    private ARUtilsManager queueManager;
    private ARUtilsManager listManager;
    private static String IP = "192.168.42.1";
    private static int PORT = 21;
    private static String LOGIN = "anonymous";
    private static String REMOTE_PATH = "internal_000";
    ...
    public boolean start()
    {
        ...
        LOCAL_PATH = Environment.getExternalStorageDirectory().getAbsolutePath() + "/BebopFlyingBanana";
        ...
        try {
            ...
            queueManager = new ARUtilsManager();
            listManager = new ARUtilsManager();
            queueManager.initWifiFtp(IP, PORT, LOGIN, "");
            listManager.initWifiFtp(IP, PORT, LOGIN, "");

            mediasDownloader.createMediasDownloader(queueManager, listManager, REMOTE_PATH, LOCAL_PATH);
        } catch (ARDataTransferException ex) {
            Log.e(TAG, "Failed: " + ex.toString());
            ex.printStackTrace();
        } catch (ARUtilsException ex) {
            Log.e(TAG, "Failed: " + ex.toString());
            ex.printStackTrace();
        }
    ...
    public void takePhoto(Button photoBt) {
        int oldNoOfMedia = -1;
        int newNoOfMedia = -1;

        // Get current number of media
        try {
            oldNoOfMedia = mediasDownloader.getAvailableMediasSync(false);
        } catch (ARDataTransferException ex) {
            Log.e(TAG, "Failed: " + ex.toString());
            ex.printStackTrace();
        }
        ...
        Toast.makeText(context, "Taking photo", Toast.LENGTH_LONG).show();
        ...
        if (sentStatus == false) {
            ARSALPrint.e(TAG, "Failed to send take photo command.");
        } else {
            Toast.makeText(context, "Photo taken", Toast.LENGTH_LONG).show();
        }

        /*
        // Check for increase in number of media
        do {
            try {
            ...
            try {
                newNoOfMedia = mediasDownloader.getAvailableMediasSync(true);
            } catch (ARDataTransferException ex) {
                Log.e(TAG, "Failed: " + ex.toString());
                ex.printStackTrace();
            }
        } while (oldNoOfMedia >= newNoOfMedia);


        // Transfer and delete photo from internal memory
        try {
            Toast.makeText(context, "Downloading photo (1/" + newNoOfMedia +")", Toast.LENGTH_LONG).show();

            for (int i = 0; i < newNoOfMedia; i++) {
                ARDataTransferMedia media = mediasDownloader.getAvailableMediaAtIndex(i);
                ARDataTransferMediasDownloaderProgressCallBack downloadProgressListener =
                        new ARDataTransferMediasDownloaderProgressCallBack();
                ARDataTransferMediasDownloaderCompletionCallBack downloaderCompletionListener =
                        new ARDataTransferMediasDownloaderCompletionCallBack(context, mediasDownloader, photoBt, newNoOfMedia);
                mediasDownloader.addMediaToQueue(media, downloadProgressListener, null, downloaderCompletionListener, null);
            }

            Thread downloaderQueueThread = new Thread(mediasDownloader.getDownloaderQueueRunnable());
            downloaderQueueThread.start();
        } catch (ARDataTransferException ex) {
            Log.e(TAG, "Failed: " + ex.toString());
            ex.printStackTrace();
        }
...
//                    ARSALPrint.e (TAG, "ReaderThread readDataWithTimeout() failed. " + netError + " bufferId: " + bufferId);
    ...
    private class VideoThread extends LooperThread
    {
        ...
        public void onloop()
        {
            ARFrame f = streamManager.getFrameWithTimeout(TIMEOUT);
            //display frame
